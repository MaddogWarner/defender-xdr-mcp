...
| C16 | Container: non-root, read-only root filesystem, dropped capabilities, `no-new-privileges`, loopback publish | `Dockerfile`, `compose.yaml`                  | **Not verified — see R2**                  |
...
- **147 unit tests across 19 files: passing.**
- Coverage: **98.89 % lines overall**; guardrails 99.15 %; audit 97.22 %.
...
- **The container has never been built or run.** `docker build`, `docker run`, the non-root assertion and the mounted-volume audit write are all untested.
...
| **R2**  | **Product has never run against a live tenant or in a container.** Unknown behaviour on first contact with real data and real throttling.  | **High** (Likely × Major)         | Extensive unit testing; documented runbook                                                                                                                                              | **Medium** after runbook completion; **High** until then             | Complete `live-test-runbook.md` in full, in a non-production or low-impact context, before any analyst-facing use. Do not waive this.                                                                                                                                                            |
