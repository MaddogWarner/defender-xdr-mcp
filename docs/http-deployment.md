...
The container health check calls the unauthenticated bare path `GET /healthz` over loopback. A healthy v1.1.1 process returns only `{"version":"1.1.1"}`. Queries, subpaths and non-GET methods do not share this Host-check bypass.
